...
**Food and energy are real costs, not decoration.** Food drains every tick regardless of colonist count; if it can't be covered, the whole living crew takes starvation damage until someone gathers again. Energy costs 1/tick per *completed* structure to stay powered; left unpaid, structures decay and can be destroyed outright, undoing both the build progress and the win condition's structure count. Metal and biomatter have no *ongoing* drain, but both are spent directly: metal on building, biomatter on repair (below).

**Repair costs biomatter, and biomatter only comes from the danger zone.** `REPAIR_STRUCTURE` used to be free — `+20` hp for nothing, no resource check at all, unlike building's metal cost. It now costs 5 biomatter from the shared pool, and biomatter has exactly one source: `alien_nest`, the same sector seeded with both starting aliens and the highest `threat_level` in the world. Before this, biomatter was tracked everywhere (every stock, every benchmark chart) and spent nowhere — pure risk for no payoff. Now a colony that lets structures decay under energy shortfall can only fix them by sending someone into the sector the aliens actually live in.

**`take_cover` blunts a hit instead of doing nothing.** It used to be a legal action with zero mechanical effect beyond raising stress — real trial dialogue showed a colonist choosing it every tick while dying anyway, worse than a no-op because the prompt used to recommend it as an escape. The prompt now says plainly that only `fire_weapon`/`retreat` end a threat, but `take_cover` itself needed a real (if weaker) effect rather than staying a trap a model could still legally pick: it now halves the damage of the *next* attack phase if the colonist is hit before acting again, and is consumed whether or not an alien actually attacks that tick — so it has to be re-chosen every tick to stay useful, and it never removes anyone from danger the way fleeing does.

**Gathering fills your own pocket, not the colony's.** `gather_resource` deposits into the colonist's personal stock, not the shared pool — nothing the colony draws on (building, structure upkeep, food upkeep) can touch it until that colonist explicitly `contribute_resources`, and only from `colony_core` — it's a walk back, not a free radio call. Contributing can be partial (`target_id="food:3"` hands over exactly 3 and quietly keeps the rest) or everything (`target_id` left null). The actual incentive to hoard: if the shared food pool runs dry, a colonist sitting on personal food quietly eats their own stash and is spared, individually, while everyone who shared everything (or never gathered) starves alongside each other regardless. Verified in real play — one real trial produced a colonist who gathered 14 food and contributed none of it, not out of calculated self-interest but because her personality kept pulling her toward exploring rather than ever circling back to share; the shared pool drained the whole time regardless of what she was personally sitting on.
