...
def _score_trial(trial):
    ...
    for a in trial["agents"]:
        ...
        if cat == "combat":
            ...
            # gather_resource fills personal_stock, not the shared pool --
            # only contribute_resources actually helps the colony, so that's
            # weighted higher than raw gathering (which could be pure
            # hoarding). Mirrors benchmark_scoring.py SCORING_VERSION 2.
            contributed = min(1.0, a.get("resources_contributed_total", 0) / 80) * 45
            gathered = min(1.0, a["resources_gathered_total"] / 100) * 15
            scores.append(round(contributed + gathered + reliability * 20 + activity * 20, 1))
        else:
...
<div class="wrap">
  ...
  <section class="panel">
    ...
    <p class="panel-sub">0-100, computed at report time from raw facts — survival scored on activity + reliability + exploration, combat on kills + health margin, economy on resources actually contributed to the colony (not just gathered — see the hoard-vs-share mechanic). See app/core/benchmark_scoring.py.</p>
    {score_chart}
  </section>
  ...
  <footer>Void Marauders benchmark harness &middot; app/core/benchmark_scoring.py SCORING_VERSION 2 &middot; generated by generate_report.py</footer>
</div>
</body>
</html>"""
